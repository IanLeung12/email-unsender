...
function monitorSendButton() {
  // Gmail's send button typically has data-tooltip="Send [Ctrl-Enter]"
  // or is within aria-label="Send"
  
  // Use a MutationObserver to catch dynamically added send buttons
...
function extractRecipients() {
  try {
    // Look for recipient fields in Gmail compose window
    const recipientElements = document.querySelectorAll('[data-addresses]');
    const recipients = [];
    
    recipientElements.forEach((elem) => {
      const addresses = elem.getAttribute('data-addresses');
      if (addresses) {
...
function extractSubject() {
  try {
    // Look for subject field in Gmail compose
...
function performUnsend(callback) {
  ...
  // TODO: Implement actual unsend via Gmail API in background worker
  chrome.runtime.sendMessage({
    action: 'unsendEmail',
    emailData: emailSendContext,
  }, (response) => {
